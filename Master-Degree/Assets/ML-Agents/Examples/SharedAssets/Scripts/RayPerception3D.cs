...
namespace MLAgents
{
    ...
    public class RayPerception3D : RayPerception
    {
        ...
        public override List<float> Perceive(float rayDistance,
            ...
            float startOffset, float endOffset, ArenaAgent arenaAgent)
        {
            ...
            {
                m_EndPosition = transform.TransformDirection(PolarToCartesian(rayDistance, angle));
                ...
                if (Physics.SphereCast(transform.position + new Vector3(0f, startOffset, 0f), 0.2f,m_EndPosition, out m_Hit, rayDistance))
                {
                    for (var i = 0; i < detectableObjects.Length; i++)
                    {
                        ...
                        if(i == 1 && (m_Hit.collider.gameObject.CompareTag(arenaAgent.tag)))
                        {
                            m_SubList[i] = 1;
                            m_SubList[detectableObjects.Length + 1] = m_Hit.distance / rayDistance; 
                            break;  
                        ...
                        if(i == 2 && !(m_Hit.collider.gameObject.CompareTag(arenaAgent.tag)))
                        {
                            m_SubList[i] = 1;
                            m_SubList[detectableObjects.Length + 1] = m_Hit.distance / rayDistance; 
                            break;  
